...
今回は、「あるマスのwon,lostを決めた時、プレーオフできなくなったらその時点で枝狩りする」として、探索をします。

枝狩りをした場合の計算量は非常に見積もりにくいです。  
でも今回はサンプルが助けてくれます。最後のケースでは、チーム数9に対して、制約的に最低限である１試合の結果しか与えられていません。これはいわゆる最悪ケースというもので、あり得る一番規模の大きい問題になります。このケースをある程度の速度で解ければ、現実的な解法と呼べるでしょう。  
